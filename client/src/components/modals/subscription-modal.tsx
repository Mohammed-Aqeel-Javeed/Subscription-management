...
export default function SubscriptionModal({ open, onOpenChange, subscription }: SubscriptionModalProps) {
  ...
  delete (subscriptionData as any).tenantId;
    ...
    onSuccess: async (data, variables) => {
      ...
      // History is automatically created by backend for both create and update
      toast({
        title: "Success",
  ...
  const tenantId = String((window as any).currentTenantId || (window as any).user?.tenantId || "");
      ...
      if (isEditing) {
        ...
        if (!validId) {
          toast({
            title: "Error",
            description: "Invalid subscription ID. Cannot update.",
            variant: "destructive",
          });
          return;
        }
        ...
        // Backend automatically creates history record for updates
      } else {
        ...
        if (res.ok && result) {
          // Backend automatically creates history record for new subscriptions
    ...
    try {
      ...
      const payload = {
        ...
        if (subId) {
          ...
          // Backend automatically creates history record for renewal
          
          // Invalidate queries to refresh data
          queryClient.invalidateQueries({ queryKey: ["/api/subscriptions"] });
          queryClient.invalidateQueries({ queryKey: ["/api/history"] });        toast({
